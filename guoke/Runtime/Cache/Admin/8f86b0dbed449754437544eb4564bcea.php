...
<html lang="en">
  ...
  <body>
			...
			<div class="panel-heading bk-bg-primary">
				<h4 class="panel-title">
					<a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion2" href="#collapse2One">
						<i class="fa fa-star"></i><a class="ziti"><?php echo ($name); ?></a>
					</a>
					<a href="<?php echo U('Admin/Group/Gjoin');?>" style="float:right" class="bk-margin-5 btn-sm">返回列表</a>
				</h4>
			</div>
			<div id="collapse2One" class="accordion-body collapse in">
				<div class="panel-body">
	
				<?php echo (htmlspecialchars_decode($res)); ?>
				</div>
			</div>
		</div>
	</div>
	...
	<h3>审核是否通过:</h3>
	<?php if(($stats == 0)): ?><button class="bk-margin-5 btn btn-default btn-circle btn-lg" type="button"><i class="fa fa-check"></i></button>
	...
	<?php else: ?>
		...
		<button class="bk-margin-5 btn btn-warning btn-circle btn-lg disabled" type="button"><i class="fa fa-times"></i></button><?php endif; ?>
	</div>
	...
	$('.btn-default').click(function(){
		...
		var he=$('.btn-warning');
		$.post(
			"<?php echo U('Admin/Group/jointo');?>",
			{id:id,T:'true'},
			function(data){
				if(data==1){
					alert(name+'小组创建成功');
					// location.href="<?php echo U('Admin/Group/index');?>";
