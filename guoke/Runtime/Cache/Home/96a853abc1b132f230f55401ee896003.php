...
<div class="gmain">
    ...
    <form class="gform" method="POST" action="<?php echo U('Home/Email/insert');?>" id="newMessage">
        <input id="csrf_token" name="csrf_token" type="hidden" value="1464780994.2##8df4a5f870abbfe76896293412daddd07c9481b7">
        
    
    
    <label for="addressee">发给</label>
    <?php if($res == '' ): ?><div class="gform-box">
            <input class="gstxt" id="addname" name="uname" type="text" value="">
            <select id="sel" name="state" style="height:25px;">
                <option value="1">普通用户</option>
                <option value="0">管理员</option>
            </select>
            <br> 
            <span id="nmsg" style="color:red;"></span>
    	    <span class="tip"></span>
        </div>
    <?php else: ?>
        <div>
            <span style="font:18px 'Microsoft Yahei';color:#525252;"><?php echo ($res["User_Nickname"]); ?></span>
            <br>
            <span class="tip"></span>
        </div><?php endif; ?>
...
<div class="gform-box">
            <script id="editor" name="msg" type="text/plain" style="width:590px;height:400px;"></script>
    <span class="tip"></span>
    ...
    <input id="captcha" name="captcha" type="hidden" value="">
        ...
        <p class="main-form-submit">
            <input type="submit" class="gnbtn-primary gform-submit" value="发送" />
        </p>
	...
	<script type="text/javascript">
		$('#sel').change(function(){
			...
			if(sel==0){
				...
				$('#addname').attr('placeholder','管理员直接输入信息');
			}else{
                $('#addname').removeAttr('readonly');
				$('#addname').removeAttr('placeholder','');
				$('#addname').val('');
			}
		})
        // 用户名失去焦点
        $('#addname').blur(function(){
            var val=$(this).val();
            var url="<?php echo U('Admin/Email/ajaxname');?>";
            if(val==''){
              $('#nmsg').html('用户名不能为空');
            }else{
