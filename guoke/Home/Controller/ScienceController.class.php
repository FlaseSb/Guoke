    ...
    public function ajaxwaterfall(){
            ...
            if ($id>0) {
                ...
                if (!empty($cpid)) {
                    ...
                    //拼接上线分类
                    $where.=" and sc_article.Display=1";
                    // var_dump($where);
                    // die;
                }else{
                    //否则,查看当前类上线分类内容
                    $where="sc_article.Fid=".$id." and sc_article.Display=1";
                }
            }else{
                //否则,查看所有上线内容
    	       $where="sc_article.Display=1";
            }
    	    // echo $count;
    	    ...
    	    foreach ($res as $k => $v) {
                ...
                //截取文章内容作为外部缩略显示
    	    	$res[$k]['Content']=substr(strip_tags(htmlspecialchars_decode($v['Content'])),0,$num).'...';
                //格式化时间
